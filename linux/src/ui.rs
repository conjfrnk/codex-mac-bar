...
use crate::model::{
    AccountRateLimitsResponse, CreditsSnapshot, DailyUsageBucket, RateLimitWindow,
    SpendControlLimitSnapshot, UsageSnapshot,
};
use crate::range::{
    Timeframe, UsageRange, format_full_tokens, format_tokens, reconcile_all_time_summary,
};
...
const RATE_TEXT_LIMIT: usize = 64;
...
struct UsagePresentation {
    ...
    average_daily_tokens: Option<i64>,
    peak_daily_tokens: Option<i64>,
    active_days: Option<usize>,
    daily_history_available: bool,
    daily_history_partial: bool,
    has_unreconciled_all_time_total: bool,
}
...
impl UsagePresentation {
    fn new(timeframe: Timeframe, range: &UsageRange, snapshot: &UsageSnapshot) -> Self {
...
fn clamped_progress_percent(value: f64) -> f64 {
    if value.is_finite() && value >= 0.0 {
        if value == 0.0 {
            0.0
        } else {
...
fn format_window_duration(minutes: Option<i64>) -> String {
...
fn reset_description(resets_at: Option<f64>, now: DateTime<Local>) -> String {
...
fn relative_reset_dates<Tz: TimeZone>(reset: &DateTime<Tz>, now: &DateTime<Tz>) -> String {
    let Some(delta_milliseconds) = reset.timestamp_millis().checked_sub(now.timestamp_millis())
    else {
        return "n/a".into();
    };
...
fn relative_age(date: DateTime<Local>, now: DateTime<Local>) -> String {
...
fn has_rate_limit_decoding_issues(response: &AccountRateLimitsResponse) -> bool {
    !response.decoding_issues.is_empty()
}

fn format_credits(credits: &CreditsSnapshot) -> String {
...
fn limit_reached_status(value: Option<&str>) -> Option<String> {
    value.map(|value| {
...
fn hero_accessible_label(total_tokens: Option<i64>) -> String {
...
fn clean_remote_value(value: &str, maximum: usize) -> String {
...
fn sanitize_single_line(message: &str, maximum: usize) -> String {
    ...
    for (inspected, character) in message.chars().enumerate() {
        ...
        if character.is_whitespace() || unsafe_format_character(character) {
